...
//    return CGRectGetWidth(self.collectionView.frame) - self.sectionInset.left - self.sectionInset.right;
    CGFloat width = CGRectGetWidth(self.collectionView.frame) - self.sectionInset.left - self.sectionInset.right;
    printf("Item width is: %f; Frame: %f, Bounds: %f, Left: %f, Right: %f\n", width, self.collectionView.frame.size.width, self.collectionView.bounds.size.width, self.sectionInset.left, self.sectionInset.right);
    return width;
}
...
{
    printf("-- invalidateLayoutWithContext - IDSC: %i, IFLA: %i, IFLDM: %i, IFLMC: %i --\n",
           context.invalidateDataSourceCounts, context.invalidateFlowLayoutAttributes,
           context.invalidateFlowLayoutDelegateMetrics, context.invalidateFlowLayoutMessagesCache);
